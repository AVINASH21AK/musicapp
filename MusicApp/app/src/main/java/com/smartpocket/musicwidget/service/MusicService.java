...
public class MusicService extends Service implements MusicPlayerCompletionListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            if (action.equals(MusicWidget.ACTION_PLAY_PAUSE)) {
                ...
                App.showLog(TAG, "song model: " + song.getTitle());
                App.showLog(TAG, "song model: " + song.getURI());
                App.showLog(TAG, "song model: " + song.getArtist());
                jumpTo(song);
            }
    ...
    public void onDestroy() {
        App.showLog(TAG, "DESTROY SERVICE");
    ...
    private void updateUI(String title, String artist, String duration, Boolean isPlaying) {
        ...
        if (title != null && artist != null && duration != null) {
            ...
            if (ActOfflineSong.layoutTextViews != null) {
                ActOfflineSong.llPlayer.setVisibility(View.VISIBLE);
                ActOfflineSong.layoutTextViews.setVisibility(View.VISIBLE);

                ActOfflineSong.textViewTitle.setText(title);
                ActOfflineSong.textViewArtist.setText(artist);
                ActOfflineSong.textViewDuration.setText(duration);
            }
            ...
            if (ActOfflineSong.layoutTextViews != null) {
                ActOfflineSong.layoutTextViews.setVisibility(View.GONE);
            }

            if (ActOfflineSong.llPlayer != null) {
                ActOfflineSong.llPlayer.setVisibility(View.GONE);
            }
        }
        ...
        if (isPlaying != null) {
            if (isPlaying) {
                ...
                if (ActOfflineSong.button_play_pause != null) {
                    ActOfflineSong.button_play_pause.setImageResource(R.drawable.ic_pause_white_36dp);
                }
            } else {
                remoteViews.setImageViewResource(R.id.button_play_pause, R.drawable.ic_play_arrow_white_36dp);
                if (ActOfflineSong.button_play_pause != null) {
                    ActOfflineSong.button_play_pause.setImageResource(R.drawable.ic_play_arrow_white_36dp);
                }
            }
        }
